...
class Settings(BaseSettings):
    ...
    FEAR_GREED_API_URL: AnyUrl | None = Field(None, description="Fear & Greed index API URL")
    COINGECKO_API_KEY: str | None = Field(None, description="Optional CoinGecko API key")

    # Database
    POSTGRES_DSN: str | None = Field(None, description="Postgres DSN (eg. postgres://user:pass@host/db)")
    # Convenience fields used across the codebase (may be derived from POSTGRES_DSN)
    DB_HOST: str | None = Field(None, description="Postgres host")
    DB_PORT: int | None = Field(None, description="Postgres port")
    DB_NAME: str | None = Field(None, description="Postgres database name")
    DB_USER: str | None = Field(None, description="Postgres user")
    DB_PASSWORD: str | None = Field(None, description="Postgres password")
    ...
    # RUN ID
    RUN_ID: str | None = Field(None, description="Explicit run id (overrides generation)")
...
# Thresholds / tuning
ANOMALY_DETECTION_THRESHOLD = getattr(settings, "ANOMALY_DETECTION_THRESHOLD", 3.0)
