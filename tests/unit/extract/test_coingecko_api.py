...
def test_fetch_success(monkeypatch):
    monkeypatch.setattr(requests, "get", lambda *a, **k: FakeResp(200, {"id": "bitcoin"}))
    # Avoid sleeping inside wrapper
    monkeypatch.setattr(cg.time, "sleep", lambda s: None)
    res = cg.fetch_coin_market_data("bitcoin")
    assert isinstance(res, dict)
    assert res["id"] == "bitcoin"
...
def test_retry_on_transient_then_success(monkeypatch):
    ...
    def fake_get(*a, **k):
        ...
        return FakeResp(200, {"id": "ethereum"})
...
def test_rate_limit_honors_retry_after_and_succeeds(monkeypatch, tmp_path):
    ...
    res = cg.fetch_coin_market_data("litecoin")
    assert res["id"] == "litecoin"
...
def test_permanent_error_writes_dead_letter(monkeypatch, tmp_path):
    # Return 404 client error -> permanent
    monkeypatch.setattr(requests, "get", lambda *a, **k: FakeResp(404, {"error": "not found"}))
    monkeypatch.setattr(cg, "DEAD_LETTER_DIR", tmp_path / "dead_letter")
    monkeypatch.setattr(cg.time, "sleep", lambda s: None)
    with pytest.raises(cg.PermanentAPIError):
        cg.fetch_coin_market_data("nonexistent-coin")
...
def test_transient_exhaustion_raises_transient(monkeypatch):
    # Always raise Timeout -> transient -> exhausted after attempts
    ...
    with pytest.raises(cg.TransientAPIError):
        cg.fetch_coin_market_data("any-coin")
